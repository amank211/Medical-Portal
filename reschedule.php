...
$appointments = array();
$sql = "Select reff_no, patient_name, Problem, appoint_time from appointments where Doctor_id = ? AND appoint_time > current_timestamp order by appoint_time";
...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
	  ...
	  <div class="collapse navbar-collapse" id="navbarNav">
		<ul class="navbar-nav">
		  ...
		  <li class = "nav-item"><a href="reschedule.php" class="nav-link"  style = "text-decoration : underline">Reschedule Appointments</a></li>
...
<script>
		...
		document.getElementById('day').innerHTML = "<th><button id = \"submit\" type=\"button\" class=\"btn btn-primary\">Select to Reschedule</button></th>";
		...
		function Refer(elem){
			if(selected != null){
				document.getElementById(selected).style.backgroundColor = "#9b59b6";
			} else{ 
				
			}
			document.getElementById("submit").innerHTML = "Reschedule App No." + elem.id;
			selected = elem.id;
		...
		Object.keys(dict_date).forEach(function(key) {
			...
			for(k = 0; k < dict_date[key].length;k++){
				...
				document.getElementById("table").rows[getindices(dict_date[key][k]).time].cells[getindices(dict_date[key][k]).day].id = dict_date[key][k][0];
				
				document.getElementById("table").rows[getindices(dict_date[key][k]).time].cells[getindices(dict_date[key][k]).day].onclick = function(){ Refer(this)};
